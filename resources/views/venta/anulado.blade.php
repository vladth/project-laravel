@extends('layouts.app')
@section("title","Facturas Anulados")
@section("styles")
    <!-- Page JS Plugins CSS -->
    <!--  Datatables  -->
...
@if(Auth::check())
	...
	<!--Contenido-->
	<div class="col-lg-12 ">         
        <h2 class="content-heading">FACTURAS ANULADAS</h2>
    </div>
                ...
                <!-- Dynamic Table Full -->
                    <div class="block">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Lista de <small> Facturas Anuladas</small></h3>
                            <a href="{{ url('venta') }}">
                        ...
                        <div class="block-content block-content-full">
                            ...
                            <table id="config" class="table table-bordered  display nowrap" width="100%">
                                ...
                                <tbody>
                                    @foreach($Fanulado as $anulado)
                                    <tr>
                                        <td>{{ ++$cont }}</td>
                                        <td>{{$anulado->updated_at}}</td>
                                        <td>{{$anulado->tipo_identificacion}}</td>
                                        <td>{{$anulado->num_venta}}</td>
                                        <td>{{$anulado->total}}</td>
                                        <td><span class="badge badge-danger">{{$anulado->estado}}</span></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
